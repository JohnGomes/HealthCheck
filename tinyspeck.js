...
class TinySpeck extends EventEmitter {
    ...
    listen(port, token) {
      ...
      dispatcher.onGet("/schedule", function (req, res) {
        debugger;
        if(req.params.channel && req.params.channel.length > 1)
            responder.schedule(req.params.channel);
        });

        //dispatcher.onPost("check-it", function (req, res) {
            //debugger;
        //});

        //dispatcher.onPost("interact", function (req, res) {
            //debugger;
        //});
